...
export default async function AdminProgramsPage() {
    ...
    // Fetch unique bureau templates (we'll fetch from bureau_programs as stand-ins for now)
        ...
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500">Control global compliance requirements, templates, and validation rules.</p>
                </div>
                <Button className="bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest text-[10px] h-9 gap-2 shadow-md shadow-red-500/10">
                    <Plus className="h-4 w-4" /> Create New Bureau
                </Button>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                <Card className="bg-white border-slate-200 text-slate-900 shadow-sm">
                    ...
                    <CardContent>
                        <div className="text-3xl font-black italic tracking-tighter text-slate-900">32</div>
                        <p className="text-[10px] text-slate-400 mt-1 uppercase font-black tracking-widest italic leading-none">Baseline Architecture: v4.2.0</p>
                    </CardContent>
                </Card>
                <Card className="bg-white border-slate-200 text-slate-900 shadow-sm">
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                        <div className="text-3xl font-black italic tracking-tighter text-emerald-600">100%</div>
                        <p className="text-[10px] text-slate-400 mt-1 uppercase font-black tracking-widest italic leading-none">All templates synced with Data Engine</p>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-4">
                    {programs?.map((program: any) => (
                        <Card key={program.id} className="bg-white border-slate-200 text-slate-900 hover:border-slate-300 transition-all shadow-sm group cursor-pointer border-l-4 border-l-transparent hover:border-l-red-500">
                            <CardContent className="p-0">
                                <Link href={`/admin/programs/${program.id}`} className="flex items-center justify-between p-6">
                                    <div className="flex items-center gap-6">
                                        <div className="h-14 w-14 rounded-2xl bg-slate-50 flex items-center justify-center border border-slate-100 shrink-0 shadow-inner group-hover:bg-white transition-colors">
                                            <span className="text-xl font-black text-slate-300 italic tracking-tighter group-hover:text-slate-900 transition-colors">{program.bureau?.slice(0, 2).toUpperCase()}</span>
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-3">
                                                <h3 className="font-black text-xl italic tracking-tight text-slate-900 group-hover:text-red-600 transition-colorsLeading-tight">{program.title}</h3>
                                                <Badge variant="outline" className="border-slate-200 text-[10px] font-black tracking-widest text-slate-400 uppercase bg-slate-50">{program.status}</Badge>
                                            </div>
                                            <div className="flex items-center gap-5 mt-2">
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                                                    Checklist Items: <span className="text-slate-900 font-black italic">12</span>
                                                </p>
                                                <div className="h-1 w-1 rounded-full bg-slate-200" />
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                                                    Validation Rules: <span className="text-slate-900 font-black italic">5</span>
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-10">
                                        <div className="flex flex-col items-end hidden sm:flex">
                                            <div className="flex -space-x-2">
                                                <div className="h-7 w-7 rounded-full border-2 border-white bg-slate-50 text-slate-400 flex items-center justify-center text-[8px] font-black tracking-tighter shadow-sm">AI</div>
                                            ...
                                            <p className="text-[9px] text-slate-300 font-black uppercase mt-1.5 tracking-widest">Controls Active</p>
                                        </div>
